...
When the viewer can manage app access, Keeper appends a platform-owned
**Administration → Members** entry to the grouped menu. It opens the same membership controls as
the header's **Access & sharing** action. Do not create a duplicate app view merely to expose app
membership; an app-owned people/profile view should contain only optional domain metadata.
